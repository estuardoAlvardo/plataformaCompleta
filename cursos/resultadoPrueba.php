...
// BUSCAMOS RESULTADO DEL LA PRUEBA REALIZADA POR ALUMNO

  $queryBuscarExamenRealizado = ("SELECT * FROM registroExamenCalificado where idExamen=:idRegistro and idUsuario=:idUsuario");
  $buscarExamenRealizado = $dbConn->prepare($queryBuscarExamenRealizado);
  $buscarExamenRealizado->bindParam(':idRegistro',$_GET['tokenRegister'], PDO::PARAM_INT);
  $buscarExamenRealizado->bindParam(':idUsuario',$_SESSION['idUsuario'], PDO::PARAM_INT);
  $buscarExamenRealizado->execute();
  $yaRealizoExamen=$buscarExamenRealizado->rowCount();
...
<html lang="es">
  <head>
    ...
    <title><?php echo $_SESSION["nombre"]; ?> | Resultado prueba</title>
...
/*estilo tiempo*/
  ...
  .card11 .face.face2 h2{
	...
	<div class="col-md-8 col-xs-8 pag-center">
         <div class="col-md-12" style="margin-top: 60px;">
              <h3 class="text-center">Prueba Calificada</h3><br>
          <div class="salida"></div>  

  <a href="<?php echo 'gestorCursoAlumno.php?tokenCourse='.$_GET['tokenCourse'].'&course='.$_GET['course']; ?>" style="font-size: 30px; border: 1px solid #3498db; margin-top:40px; margin-bottom: 50px;  margin-left: 10px; height: 50px;"  class="btn btn-default botonAgg-1">Volver a mi curso</a>
  </form>
         ...
         <?php while(@$row2=$obtenerDetalleCuestionario->fetch(PDO::FETCH_ASSOC)){  
          ...
          $_SESSION['punteoTotal']=$row2['punteo'];
 ...
 ?>
      <div class="col-md-12 cajaDescripcion" style="min-height:100px; text-align: left; border-radius: 10px; border:2px solid; border-image: linear-gradient(to right, #C6426E, #642B73)1; margin-bottom: 50px;">
        ...
        <div style="margin-left: 240px; margin-top: 60px;">
       ...
       <h4 style="font-weight: bold;">Fecha Limite: <span  style="font-weight: normal;"><?php echo $row2['fechaExamen']; ?></span> </h4>
        ...
        <h4 style="font-weight: bold;">Nombre Alumno: <span style="font-weight: normal;"><?php echo strtoupper($_SESSION['nombre'])." ".strtoupper($_SESSION['apellido']); ?></span> </h4>
                 ...
                 <?php while(@$datosResultado=$buscarExamenRealizado->fetch(PDO::FETCH_ASSOC)){ 

                      $_SESSION['idQuizCalificado']=$datosResultado['idCuestionarioCalificado'];
...
<div class="col-md-4 cajaDescripcion" style="min-height: 200px; margin-top: 30px; margin-left: 35%; background:-webkit-gradient(radial, 220 0, 0, 165 173, 468, from(#8E4D61), to(#05455E));
 border-radius: 5px; color:white; margin-bottom: 50px;">
          <h4 style="text-align: center;">Resultado Prueba</h4>
          ...
          <div class="col-md-12 botonAgg-1" style="margin-left:23%; height: 150px; width: 150px; border-radius: 100%; background-color: #2ecc71; padding-top: 15px;">
            <h3 style="font-size: 40pt; margin-left: 20px;"><?php echo $datosResultado['punteoObtenido']; ?>/<?php echo $_SESSION['punteoTotal']; ?></h3>
          </div>           
         </div> 
...
//calificar y validar respuestas correctas e incorrectas
   $buscarOptM=("SELECT * from cuestionarioROpcionMultiple where idQuizCalificado=:idQuizCalificado and idUsuario=:idUsuario and idPregunta=:idPregunta");
   $buscarOpcionMultipleCalificado = $dbConn->prepare($buscarOptM);
   $buscarOpcionMultipleCalificado->bindparam(':idQuizCalificado', $_SESSION['idQuizCalificado']);
   $buscarOpcionMultipleCalificado->bindparam(':idUsuario', $_SESSION['idUsuario']);
   $buscarOpcionMultipleCalificado->bindparam(':idPregunta', $datosOpcionMultiple['idRegistro']);

   $buscarOpcionMultipleCalificado->execute();

   while(@$datosExamenCalificado=$buscarOpcionMultipleCalificado->fetch(PDO::FETCH_ASSOC)){  
   
   // echo 'idPregunta '.$datosExamenCalificado['idPregunta'].' Respuesta Elejida '.$datosExamenCalificado['respuesta'].' <br>';
...
<div class="card boxCard col-md-12" style="text-align: left; margin-bottom: 20px; pointer-events:none;">
 ...
 <?php } ?>
   ...
   <ul>
    ...
    <input value="0" <?php if($datosExamenCalificado['respuesta']==0){ echo 'checked';  } ?>  type="radio" name="<?php echo 'multiple'.$datosOpcionMultiple['idRegistro']; ?>"  style=""  />
    <li >


      <input value="1" type="radio" <?php if($datosExamenCalificado['respuesta']==1){ echo 'checked';  } ?>  name="<?php echo 'multiple'.$datosOpcionMultiple['idRegistro']; ?>" id="one" style=""  />
    ...
    <li>
      <input type="radio" value="2" <?php if($datosExamenCalificado['respuesta']==2){ echo 'checked';  } ?>  name="<?php echo 'multiple'.$datosOpcionMultiple['idRegistro']; ?>" id="two" />
      <label ><?php echo $datosOpcionMultiple['respuesta2']; ?></label>
      
      <div class="check"></div>
    </li>
    <li>
      <input type="radio" value="3" <?php if($datosExamenCalificado['respuesta']==3){ echo 'checked';  } ?>  name="<?php echo 'multiple'.$datosOpcionMultiple['idRegistro']; ?>" id="tree" />
    ...
    <?php if($datosExamenCalificado['respuesta']==$datosOpcionMultiple['respuestaCorrecta']){ 

      ?>
    <img src="../img/leido1.png" width="50" height="50" style="margin-left: 90%;">
  <?php }else{ ?>
        <img src="../img/error.png" width="50" height="50" style="margin-left: 90%;">
...
<?php
   ...
   while(@$datosEmparejamiento=$buscarEmparejamiento->fetch(PDO::FETCH_ASSOC)){  
    ...
    $buscarEmpRegistro=("SELECT * from cuestionarioREmparejamiento where idQuizCalificado=:idQuizCalificado and idUsuario=:idUsuario and idPregunta=:idPregunta");
   $buscarEmpPregunta = $dbConn->prepare($buscarEmpRegistro);
   $buscarEmpPregunta->bindparam(':idQuizCalificado', $_SESSION['idQuizCalificado']);
   $buscarEmpPregunta->bindparam(':idUsuario', $_SESSION['idUsuario']);
   $buscarEmpPregunta->bindparam(':idPregunta', $datosEmparejamiento['idRegistro']);
   $buscarEmpPregunta->execute();

      while(@$respuestaEmparejamiento=$buscarEmpPregunta->fetch(PDO::FETCH_ASSOC)){  
...
<div class="boxCard col-md-12" style="text-align: left; margin-bottom: 20px; pointer-events: none;">
  <div style="display: inline-block; border: 3px solid white; border-radius: 20rem; color: white; text-align: center; padding: 0.5rem; box-shadow: rgba(0, 0, 0, 0.15) 0px 1px 3px 0px; font-weight: 600; min-width: 4rem; font-size: 2rem; background: linear-gradient(to right, #C6426E, #642B73); position: absolute; margin-top: 12%; margin-left: 90%; "><?php echo @$noPreguntaOpcion; ?></div>
 ...
 ?>
    <div class="col-md-6" style="margin-bottom: 20px;"> 
      <select class="form-control" name="<?php echo 'pregunta'.$datosEmparejamiento['idRegistro'].'opt1'; ?>" id="selectTema" required>
            <option value="0" <?php if($respuestaEmparejamiento['opcion1']==0){ echo 'selected';} ?>>No respondió</option>
            <option value="<?php echo $datosEmparejamiento['respuesta1']; ?>" <?php if($respuestaEmparejamiento['opcion1']==$datosEmparejamiento['respuesta1']){ echo 'selected';} ?> ><?php echo $datosEmparejamiento['respuesta1']; ?></option>
            <option value="<?php echo $datosEmparejamiento['respuesta1']; ?>" <?php if($respuestaEmparejamiento['opcion1']==$datosEmparejamiento['respuesta2']){ echo 'selected';} ?>  ><?php echo $datosEmparejamiento['respuesta2']; ?></option>
            <option value="<?php echo $datosEmparejamiento['respuesta3']; ?>" <?php if($respuestaEmparejamiento['opcion1']==$datosEmparejamiento['respuesta3']){ echo 'selected';} ?> ><?php echo $datosEmparejamiento['respuesta3']; ?></option>
            <option value="<?php echo $datosEmparejamiento['respuesta4']; ?>" <?php if($respuestaEmparejamiento['opcion1']==$datosEmparejamiento['respuesta4']){ echo 'selected';} ?> ><?php echo $datosEmparejamiento['respuesta4']; ?></option>
      </select>
      <?php if($respuestaEmparejamiento['opcion1']==$datosEmparejamiento['respuesta1']){ ?>
      <img src="../img/leido1.png" width="20" height="20">

    <?php }else{?>
            <img src="../img/error.png" width="20" height="20">
    <?php }?>
 ...
 ?>
    <div class="col-md-6" style="margin-bottom: 20px;"> 
      ...
      <select class="form-control" name="<?php echo 'pregunta'.$datosEmparejamiento['idRegistro'].'opt2'; ?>" required>
            <option value="0" <?php if($respuestaEmparejamiento['opcion2']==0){ echo 'selected';} ?> >No respondió </option>
            <option value="<?php echo $datosEmparejamiento['respuesta1']; ?>" <?php if($respuestaEmparejamiento['opcion2']==$datosEmparejamiento['respuesta1']){ echo 'selected';} ?> ><?php echo $datosEmparejamiento['respuesta1']; ?></option>
            <option value="<?php echo $datosEmparejamiento['respuesta2']; ?>" <?php if($respuestaEmparejamiento['opcion2']==$datosEmparejamiento['respuesta2']){ echo 'selected';} ?>  ><?php echo $datosEmparejamiento['respuesta2']; ?></option>
            <option value="<?php echo $datosEmparejamiento['respuesta3']; ?>" <?php if($respuestaEmparejamiento['opcion2']==$datosEmparejamiento['respuesta3']){ echo 'selected';} ?>  ><?php echo $datosEmparejamiento['respuesta3']; ?></option>
            <option value="<?php echo $datosEmparejamiento['respuesta4']; ?>" <?php if($respuestaEmparejamiento['opcion2']==$datosEmparejamiento['respuesta4']){ echo 'selected';} ?>  ><?php echo $datosEmparejamiento['respuesta4']; ?></option>
      </select>

       <?php if($respuestaEmparejamiento['opcion2']==$datosEmparejamiento['respuesta2']){ ?>
      <img src="../img/leido1.png" width="20" height="20">
 ...
 ?>
    <div class="col-md-6" style="margin-bottom: 20px;"> 
      ...
      <select class="form-control" name="<?php echo 'pregunta'.$datosEmparejamiento['idRegistro'].'opt3'; ?>" id="selectTema" required>
            <option value="0" <?php if($respuestaEmparejamiento['opcion3']==0){ echo 'selected';} ?>>No respondió</option>
            <option value="<?php echo $datosEmparejamiento['respuesta1']; ?>" <?php if($respuestaEmparejamiento['opcion3']==$datosEmparejamiento['respuesta1']){ echo 'selected';} ?> ><?php echo $datosEmparejamiento['respuesta1']; ?></option>
            <option value="<?php echo $datosEmparejamiento['respuesta2']; ?>" <?php if($respuestaEmparejamiento['opcion2']==$datosEmparejamiento['respuesta2']){ echo 'selected';} ?> ><?php echo $datosEmparejamiento['respuesta2']; ?></option>
            <option value="<?php echo $datosEmparejamiento['respuesta3']; ?>" <?php if($respuestaEmparejamiento['opcion3']==$datosEmparejamiento['respuesta3']){ echo 'selected';} ?> ><?php echo $datosEmparejamiento['respuesta3']; ?></option>
            <option value="<?php echo $datosEmparejamiento['respuesta4']; ?>" <?php if($respuestaEmparejamiento['opcion3']==$datosEmparejamiento['respuesta4']){ echo 'selected';} ?> ><?php echo $datosEmparejamiento['respuesta4']; ?></option>
      </select>

      <?php if($respuestaEmparejamiento['opcion3']==$datosEmparejamiento['respuesta3']){ ?>
      <img src="../img/leido1.png" width="20" height="20">
 ...
 ?>
    <div class="col-md-6" style="margin-bottom: 20px;"> 
      ...
      <select class="form-control" name="<?php echo 'pregunta'.$datosEmparejamiento['idRegistro'].'opt4'; ?>" id="selectTema" required>
            <option value="0" <?php if($respuestaEmparejamiento['opcion4']==0){ echo 'selected';} ?> >No respondió</option>
            <option value="<?php echo $datosEmparejamiento['respuesta1']; ?>" <?php if($respuestaEmparejamiento['opcion4']==$datosEmparejamiento['respuesta1']){ echo 'selected';} ?> ><?php echo $datosEmparejamiento['respuesta1']; ?></option>
            <option value="<?php echo $datosEmparejamiento['respuesta2']; ?>" <?php if($respuestaEmparejamiento['opcion4']==$datosEmparejamiento['respuesta2']){ echo 'selected';} ?> ><?php echo $datosEmparejamiento['respuesta2']; ?></option>
            <option value="<?php echo $datosEmparejamiento['respuesta3']; ?>" <?php if($respuestaEmparejamiento['opcion4']==$datosEmparejamiento['respuesta3']){ echo 'selected';} ?> ><?php echo $datosEmparejamiento['respuesta3']; ?></option>
            <option value="<?php echo $datosEmparejamiento['respuesta4']; ?>" <?php if($respuestaEmparejamiento['opcion4']==$datosEmparejamiento['respuesta4']){ echo 'selected';} ?> ><?php echo $datosEmparejamiento['respuesta4']; ?></option>
      </select>

            <?php if($respuestaEmparejamiento['opcion4']==$datosEmparejamiento['respuesta4']){ ?>
      <img src="../img/leido1.png" width="20" height="20">
...
<?php
   ...
   while(@$datosRespuestaCorta=$buscarRespCorta->fetch(PDO::FETCH_ASSOC)){  
    ...
    $buscarRCorta1=("SELECT * from cuestionarioRRespuestaCorta where idQuizCalificado=:idQuizCalificado and idUsuario=:idUsuario and idPregunta=:idPregunta");
   $buscarRpCorta = $dbConn->prepare($buscarRCorta1);
   $buscarRpCorta->bindparam(':idQuizCalificado', $_SESSION['idQuizCalificado']);
   $buscarRpCorta->bindparam(':idUsuario', $_SESSION['idUsuario']);
   $buscarRpCorta->bindparam(':idPregunta', $datosRespuestaCorta['idRegistro']);
   $buscarRpCorta->execute();

      while(@$respuestaRCorta=$buscarRpCorta->fetch(PDO::FETCH_ASSOC)){  

        $porcentajeRespuesta=similar_text($datosRespuestaCorta['textoComparar'], $respuestaRCorta['respuestaAlumno']);


    ?>
<div class="card col-md-12" style="min-height: 250px; margin-bottom: 50px; pointer-events: none;">
...
<div class="recodinggN" id="<?php echo 'on'.$datosRespuestaCorta['idRegistro']; ?>" title="Graba tú respuesta" style="cursor: pointer; padding-top:3px; padding-left: 5px; width: 50px; height: 50px; border-radius: 100%; margin-top: 10px; background-color: #e67e22; margin-left: 40%;" onclick="quizRespuesta(this.id)"><img src="../img/micro.png" width="40" height="40" ></div>
      ...
      <input type="text" name="<?php echo 'input'.$datosRespuestaCorta['idRegistro']; ?>" id='<?php echo 'span-preview'.$datosRespuestaCorta['idRegistro']; ?>' value="<?php echo $respuestaRCorta['respuestaAlumno']; ?>" style=" border-radius: 5px; height: 50px;">
